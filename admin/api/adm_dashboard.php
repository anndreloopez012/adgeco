...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "insert_home") {
        ...
        $var_consulta = "UPDATE about SET inf_general = '$inf_general' ,img_mision = '$img_mision',ico_mision = '$ico_mision' ,title_mision = '$title_mision' ,content_mision = '$content_mision' ,img_plan = '$img_plan' ,ico_plan = '$ico_plan' ,title_plan = '$title_plan' ,content_plan = '$content_plan' ,img_vision = '$img_vision' ,ico_vision = '$ico_vision',title_vision = '$title_vision',content_vision = '$content_vision' WHERE id = 1";
        $val = 1;
        if (mysqli_query($link, $var_consulta)) {
            $arrInfo['status'] = $val;
        } else {
        ...
        print json_encode($arrInfo);
        die();
    } else if ($strTipoValidacion == "insert_clientes") {
        header('Content-Type: application/json');
        ...
        $var_consulta = "INSERT INTO clients(url,img,nombre) VALUES ('$url','$img','$nombre')";
        ...
        print json_encode($arrInfo);
        die();
    } else if ($strTipoValidacion == "delete_clientes") {
        header('Content-Type: application/json');
        $idClientes = isset($_POST["idClientes"]) ? $_POST["idClientes"]  : ' ';
        $val = 1;
        $var_consulta = "DELETE FROM clientes WHERE id = $idClientes;";
        if (mysqli_query($link, $var_consulta)) {
            $arrInfo['status'] = $val;
        } else {
            $arrInfo['status'] = 0;
...
?>
    ...
    <?php
        die();
    } else if ($strTipoValidacion == "dibujo_tabla_clientes") {
        ...
        $qTMP = mysqli_query($link, $var_consulta);
        while ($rTMP = mysqli_fetch_assoc($qTMP)) {
            $arrClientes[$rTMP["id"]]["id"] = $rTMP["id"];
            $arrClientes[$rTMP["id"]]["nombre"] = $rTMP["nombre"];
        }
        mysqli_free_result($qTMP);
    ?>
        <table class="table">
            ...
            <tbody>
                ...
                if (is_array($arrClientes) && (count($arrClientes) > 0)) {
                    reset($arrClientes);
                    $intContador = 1;
                    foreach ($arrClientes as $rTMP['key'] => $rTMP['value']) {
                ?>
                        <tr>
                            <th><?php echo  $rTMP["value"]['nombre']; ?></th>
                            <th class="table-info" style="width:5%">
                                <i class="fad fa-user-minus " style="cursor:pointer;" id="delete" onclick="fntSelectDeleteClientes('<?php print $intContador; ?>'); "></i>
                            </th>
                        </tr>
