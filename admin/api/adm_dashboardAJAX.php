<script>
    ...
    function fntInsertClientes() {

        alertify.confirm('Dashboard', 'Seguro que desea continuar? ', function() {
            var datos = $('#form_data_clientes').serialize();
            //alert(datos);
            //return false;
            document.getElementById("loading-screen").style.display = "block";
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "dashboard.php?ajax=true&validaciones=insert_clientes",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            alertify.alert('Aviso', 'Datos cargados correctamente');
                            $("#form_data_clientes")[0].reset();
                            //location.reload(); 
                            document.getElementById("loading-screen").style.display = "none";
                        }
                    } else {
    ...
    function fntDeleteClientes() {
        ...
        alertify.confirm('Dashboard', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "dashboard.php?ajax=true&validaciones=delete_clientes",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            alertify.alert('Aviso', 'Datos cargados correctamente');
    ...
    function fntDibujoClientes() {
        //alert(strPais + "                                  strPais");
        document.getElementById("loading-screen").style.display = "block";

        $.ajax({

            url: "dashboard.php?validaciones=dibujo_tabla_clientes",
            data: {},
            async: true,
            global: false,
            type: "post",
            ...
            success: function(data) {
                ...
                $("#dibujo_tabla_clientes_").html("");
                $("#dibujo_tabla_clientes_").html(data);
                document.getElementById("loading-screen").style.display = "none";
                return false;
            }
        });
    ...
    function fntSelectDeleteClientes(intParametro) {
        intParametro = !intParametro ? 0 : intParametro;
        if (intParametro > 0) {
            var strId = $("#hidId_" + intParametro).val();
            // alert(strDPI + "                         strDPI");
            $("#idClientes").val(strId);
        }
        fntDeleteClientes()
    }
